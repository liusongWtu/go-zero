...
import (
	...
	modelutil "sysconfig/tools/go/gozero/model/sql/util"
...
func (g *defaultGenerator) StartFromInformationSchema(tables map[string]*model.Table, withCache, strict bool) error {
	...
	for _, each := range tables {
		...
		apiTsCode, err := g.genTsApi(*table, g.requestPath)
		if err != nil {
			return err
		}
		fmt.Println(apiTsCode)
...
func (g *defaultGenerator) genTsApi(in parser.Table, requestPath string) (string, error) {
	text, err := pathx.LoadTemplate(category, apiTsTemplateFile, template.ApiTs)
	if err != nil {
		return "", err
	}
	...
	fieldCode, err := genTsFields(table, in.Fields)
	if err != nil {
		return "", err
	}
...
func (g *defaultGenerator) createFile(modelList map[string]*codeTuple) error {
	...
	for tableName, codes := range modelList {
		...
		err = os.WriteFile(filename, []byte(codes.apiCode), os.ModePerm)
		if err != nil {
			return err
		}
		...
		err = os.WriteFile(filename, []byte(codes.storeCode), os.ModePerm)
		if err != nil {
			return err
		}
	}
