...
// genEditorFormRules 生成/editor/form/formRules.ts文件
func (g *defaultGenerator) genEditorFormRules(in parser.Table) (codeFile, error) {
	text, err := pathx.LoadTemplate(category, editorFormRulesTemplateFile, template.EditorFormRules)
	if err != nil {
		return codeFile{}, err
	}
	...
	return codeFile{
		filename: "src/views/" + strings.ReplaceAll(in.Name.Lower(), "_", "") + "/editor/form/formRules.ts",
